...
var __reExport = (target2, module2, copyDefault, desc) => {
...
var source = (0, import_core.getInput)("source") || "test/a.txt";
var target = (0, import_core.getInput)("target") || "test/b.txt";
function run() {
  if (!(0, import_fs.existsSync)(source)) {
    (0, import_core.setFailed)(`${source} does not exist`);
    return;
  }
  let fileAsString = (0, import_fs.readFileSync)(source, "utf8");
  for (const key of Object.keys(process.env)) {
    fileAsString = fileAsString.replace(`{${key}}`, process.env[key]);
  }
  console.log(`Wrote replacements from ${source} into ${target}`);
  (0, import_fs.writeFileSync)(target, fileAsString);
}
run();
